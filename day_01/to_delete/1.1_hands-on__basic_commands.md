...
1. Log in to your laptops and connect to the Wi-Fi.
2. Open a terminal.
...
Each program is shipped with a __manual__ (also known as help page or documentation). The documentation of a command is accessible via `man` or `--help`.
...
Please bear in mind that all Linux-based systems are __case-sensitive__, indicating that `CLEAR` ≠ `clear`. Be smart and use __auto-completion__ to avoid typos! It makes your life much easier.